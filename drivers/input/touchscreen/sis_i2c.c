...
static void sis_ts_work_func(struct work_struct *work)
{
	...
	uint8_t i = 0, fingers = 0;
	uint8_t px = 0, py = 0, pstatus = 0;
	unsigned short report_px=0,report_py=0;
	uint8_t p_area = 0;
...
// add 
	...
	for (i = 0; i < fingers; i++) // fingers 10 =  0 ~ 9
	{
		...
		TPInfo->pt[i].id = (buf[pstatus + 1]);
		report_px=((buf[px] & 0xff) | ((buf[px + 1] & 0xff)<< 8));
		report_py=((buf[py] & 0xff) | ((buf[py + 1] & 0xff)<< 8));
		TPInfo->pt[i].x =SIS_MAX_X- report_px*SIS_MAX_X/4095;
        TPInfo->pt[i].y = SIS_MAX_Y-report_py*SIS_MAX_Y/4095;      
	}
...
static int initial_irq(void)
{
	...
	/* TODO */
	ret = gpio_request(GPIO_IRQ, "tp_irq");	// ex. GPIO_133 for interrupt mode
	if (ret < 0)
	{
	...
	struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	ts->input_dev->name = DEVICE_NAME;//"sis_touch";//"SiS9200-i2c-touchscreen";
	...
	//else
	{
		//gpio_direction_input(82);//it's done in board init.
