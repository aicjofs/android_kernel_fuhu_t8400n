...
struct i2c_client *ft5x06_i2c_client=NULL; 

static struct regulator *keenhi_t40_touch_vddio = NULL;

static void focal_tp_power_supply(bool state)//true:vdd power on,false:vdd power off
{
  int ret=0;
  printk("=========================%s\n",__func__);
...
static int focal_tp_enable(struct input_dev *input_dev)
{
	struct novatek_ts_data *ts;
    	//ts = container_of(input_dev, struct novatek_ts_data, input_dev);
	printk("focal_tp_will_resume!\n");
	focal_tp_power_supply(true);
	enable_irq(ft5x06_i2c_client->irq);
		
	return 1;
}
static int focal_tp_disable(struct input_dev *input_dev)
{
	struct novatek_ts_data *ts;
    	//ts = container_of(input_dev, struct novatek_ts_data, input_dev);
	printk("focal_tp_will_suspend!\n");
	disable_irq(ft5x06_i2c_client->irq);
	focal_tp_power_supply(false);
	return 1;
...
static int ft5x0x_read_Touchdata(struct ft5x0x_ts_data *data)
{
	...
	for (i = 0; i < CFG_MAX_TOUCH_POINTS; i++) {
		...
		//printk("pointid=%d,%d,%d,%d \n",pointid,event->au16_x[i],event->au16_y[i],event->au8_touch_event[i] );
	}
...
//#reed		tmp=0;
//#reed		tmp=event->au16_x[i];
//#reed		event->au16_x[i]=event->au16_y[i];
//#reed		event->au16_y[i]=tmp;
			   ...
			   const struct i2c_device_id *id)
{
	...
	input_dev->enable=focal_tp_enable;
	input_dev->disable=focal_tp_disable;
...
static void ft5x0x_ts_suspend(struct early_suspend *handler)
{
	...
	printk(&ts->client->dev, "[FTS]ft5x0x suspend\n");
	if(!ft5x06_i2c_client)
		return;
	focal_tp_power_supply(false);
	disable_irq(ft5x06_i2c_client->irq);
}
...
//#reed	.suspend = ft5x0x_ts_suspend,
//#reed	.resume = ft5x0x_ts_resume,
	.driver = {
		   .name = FT5X0X_NAME,
		   .owner = THIS_MODULE,
		   },
