...
static void gt9xx_tp_power_supply(bool state)//true:vdd power on,false:vdd power off
{
  int ret=0;
  printk("=========================%s\n",__func__);
  if(machine_is_terra10()||machine_is_terra7()||machine_is_t8400n()||machine_is_tb610n()|| machine_is_keenhi01n())
  {
...
static void gtp_touch_down(struct goodix_ts_data* ts,s32 id,s32 x,s32 y,s32 w)
{
    ...
    input_report_abs(ts->input_dev, ABS_MT_TOUCH_MAJOR, w);
    input_report_abs(ts->input_dev, ABS_MT_WIDTH_MAJOR, w);
    input_report_abs(ts->input_dev, ABS_MT_TRACKING_ID, id);//坐标匹配
...
Output:
    None.
*********************************************************/
static void goodix_ts_work_func(struct work_struct *work)//平时开机后(上电后)处理行数
{
...
*********************************************************/
static irqreturn_t goodix_ts_irq_handler(int irq, void *dev_id)//中断处理函数
{
    ...
    queue_work(goodix_wq, &ts->work);//工作队列，与工作函数绑定
...
Output:
    None.
*******************************************************/
void gtp_reset_guitar(struct i2c_client *client, s32 ms)//开机时序
{
#if GTP_COMPATIBLE_MODE
    struct goodix_ts_data *ts = i2c_get_clientdata(client);
#endif    
    ...
    GTP_DEBUG_FUNC();
    GTP_INFO("Guitar reset");//申请到GTP_RST_PORT，开机时序
    GTP_GPIO_OUTPUT(GTP_RST_PORT, 0);   // begin select I2C slave addr
    msleep(ms);                         // T2: > 10ms
    // HIGH: 0x28/0x29, LOW: 0xBA/0xBB
    GTP_GPIO_OUTPUT(GTP_INT_PORT, client->addr == 0x14);//client->addr==5D!=14，选址 BA/BB， GTP_GPIO_OUTPUT(GTP_INT_PORT,0)
    ...
    gtp_int_sync(50);  //int同步函数；ic复位完后，int一直为高，由主控发送低电平为IC的int,规定50ms（注释后触摸失效）
#if GTP_ESD_PROTECT//esd有2个寄存器8040=aa,8041
...
static s32 gtp_init_panel(struct goodix_ts_data *ts)
{
    ...
    u8 sensor_id = 0; 
    //不同cfg文件对应不同的模组厂
    u8 cfg_info_group1[] = CTP_CFG_GROUP1;//sensor_id=0 //u8最大值2的8次方
    u8 cfg_info_group2[] = CTP_CFG_GROUP2;//sensor_id=1
...
#endif
    {
        ret = gtp_i2c_read_dbl_check(ts->client, 0x41E4, opr_buf, 1);//41E4固件校验码，正确的BE
        if (SUCCESS == ret) 
        {
            if (opr_buf[0] != 0xBE)
            {
    ...
    else
    {
        ...
        msleep(50);
    #endif
        ret = gtp_i2c_read_dbl_check(ts->client, GTP_REG_SENSOR_ID, &sensor_id, 1);//读1个字节保存到sensor_id对应的地址
        if (SUCCESS == ret)
        {
            if (sensor_id >= 0x06)//sensor_id=0~5
            {
    ...
    if (ts->gtp_cfg_len < GTP_CONFIG_MIN_LENGTH)//915 186个字节
    {
    ...
    {
		//驱动的配置是否下发到IC里面
        ret = gtp_i2c_read_dbl_check(ts->client, GTP_REG_CONFIG_DATA, &opr_buf[0], 1);//GT915->0x8047地址开始186字节，读取版本号
        ...
        if (ret == SUCCESS)
        {
            ...
            if (opr_buf[0] < 90)    
            {
                ...
                ts->fixed_cfg = 0;//0要发配置，1不要发配置
            }
            else   //版本号大于等于90，驱动不下发配置     // treated as fixed config, not send config
            {
    ...
    memcpy(&config[GTP_ADDR_LENGTH], send_cfg_buf[sensor_id], ts->gtp_cfg_len);//sendor_id=0配置拷贝到config中
    ...
    if (ret < 0)
    {
        ...
        ts->abs_x_max = GTP_MAX_WIDTH;//分辨率
        ts->abs_y_max = GTP_MAX_HEIGHT;
        ts->int_trigger_type = GTP_INT_TRIGGER;//触发方式
    }
...
static s8 gtp_request_io_port(struct goodix_ts_data *ts)
{
    ...
    {
        GTP_GPIO_AS_INT(GTP_INT_PORT);  //设置为输入，作为中断；输出则为高低电平
    ...
    GTP_GPIO_AS_INPUT(GTP_RST_PORT);//手机主板只能接受信号，不能随便发出信号；IC发出驱动信号

    gtp_reset_guitar(ts->client, 20);//复位IC
...
Output:
    Executive outcomes. 
        0: succeed.
*******************************************************/
static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)  //client,id从哪里来、
{
    s32 ret = -1;
    struct goodix_ts_data *ts;
	//ts->use_irq=1,说明我们使用中断，一定要kzalloc
    u16 version_info;
    ...
    if (!i2c_check_functionality(client->adapter, I2C_FUNC_I2C)) //检测总线是否准备好
    {
        GTP_ERROR("I2C check functionality failed.");
        return -ENODEV;
    }
    ts = kzalloc(sizeof(*ts), GFP_KERNEL);//申请内存空间，大小为结构体goodix_ts_data的大小，权限GFP_KERNEL
    if (ts == NULL)//容错机制
    {
    ...
    memset(ts, 0, sizeof(*ts));//内存操作函数，结构体goodix_ts_data全部初始化为0
    INIT_WORK(&ts->work, goodix_ts_work_func);//绑定，工作ts->work，函数名goodix_ts_work_fun实现功能报点，并上报android系统
    ts->client = client;
    spin_lock_init(&ts->irq_lock);          // 2.6.39 later，自旋锁，
    // ts->irq_lock = SPIN_LOCK_UNLOCKED;   // 2.6.39 & before
#if GTP_ESD_PROTECT//条件编译、减少系统负载，每隔2s看一下IC是否处于异常，异常就复位IC
    ...
    ts->gtp_rawdiff_mode = 0;//原始值差值模式为0，默认为坐标模式

    ret = gtp_request_io_port(ts);//申请IO口资源
    if (ret < 0)
    {
    ...
    ret = gtp_request_input_dev(ts);//申请输入设备(输入子系统)
    if (ret < 0)
    {
        GTP_ERROR("GTP request input dev failed");
    }
    
    ret = gtp_request_irq(ts); //申请中断
    if (ret < 0)
    {
...
static void gtp_esd_check_func(struct work_struct *work)
{
    ...
    for (i = 0; i < 3; i++)
    {
        ...
        { 
            ...
            {
				//每一个主循环IC都对0x8041进行清除，成FF
				//8040一定要为aa才是正确的，8041一定不是aa才是正确的
                // IC works abnormally..
...
static struct i2c_driver goodix_ts_driver = {
    .probe      = goodix_ts_probe,//第2步执行，I2C扫描函数
...
{
    s32 ret;
gt9xx_tp_power_supply(true);
    GTP_DEBUG_FUNC();   
    GTP_INFO("GTP driver installing...");//打印开机信息
    goodix_wq = create_singlethread_workqueue("goodix_wq");//创建单线程，提取坐标点，上报
    if (!goodix_wq)//容错机制
    {
...
//条件编译
#if GTP_ESD_PROTECT//每隔2秒，驱动看一下IC是否进入ESD失效模式，
    INIT_DELAYED_WORK(&gtp_esd_check_work, gtp_esd_check_func);
    gtp_esd_check_workqueue = create_workqueue("gtp_esd_check");
#endif
    ret = i2c_add_driver(&goodix_ts_driver);//注册（加载）I2C设备
    return ret; //开关机才再次执行此函数
}
...
late_initcall(goodix_ts_init);//开机后第一个运行的函数, 优先级7
//module_init(goodix_ts_init);//优先级6
module_exit(goodix_ts_exit);//关机后运行的函数
