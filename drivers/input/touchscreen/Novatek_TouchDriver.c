...
#include<linux/nvctp_BinaryFile_edt_terra10.h>
#include<linux/nvctp_BinaryFile_edt_t8400n.h>
#include<linux/nvctp_BinaryFile_wistron_t8400n.h>
#include<linux/nvctp_BinaryFile_ttl_t8400n.h>
#include<linux/nvctp_BinaryFile_racee_terra10.h>
...
static int novatek_init_panel(struct novatek_ts_data *ts)
{
	...
	if(machine_is_nabi2_3d()|| machine_is_nabi_2s() ||machine_is_qc750() ||  machine_is_itq700() || machine_is_itq701() || machine_is_n750() || machine_is_wikipad())
	{
		ts->abs_x_max = SCREEN_MAX_WIDTH_3D;
		ts->abs_y_max = SCREEN_MAX_HEIGHT_3D;
	} 
	else if(machine_is_nabi2_xd() || machine_is_n1010()||machine_is_ns_14t004()||machine_is_itq100()||
...
#endif
	i2c_write_bytes(ts->client, write_cmd, 3);
		...
		//printk("-------input_x = %d,input_y = %d-------\n", input_x, input_y);
		  switch(touch_type)
		  {
			...
			case CNT_TOUCH_ITO:
				...
				break;
			case RACEE_TOUCH:
				input_x = input_x *1920/(2752);	
				input_y = input_y *1200/(1664);
				break;
			case QTW_TOUCH:
				if(machine_is_t8400n()|| machine_is_keenhi01n())
				{
					input_x =input_x *1920/(2816);//1280 1792
					input_y =1200-input_y *1200/(1792);//800 980
					tmp = input_x;
					input_x = input_y;
					input_y = tmp;
				}
			...
			case TTL_TOUCH:
			case WISTRON_TOUCH:
				input_x =input_x *1920/1920;//(2773);//1280 1792
				input_y =1200-input_y *1200/1200;//(1685);//800 980
				tmp = input_x;
				input_x = input_y;
				input_y = tmp;
				break;
...
//					input_y =input_y *SCREEN_MAX_WIDTH_3D/(TOUCH_MAX_HEIGHT_EDT_2S);
  				...
  				//input_x =input_x *SCREEN_MAX_HEIGHT_3D/(TOUCH_MAX_WIDTH_EDT_2S);	
  				//input_y =input_y *SCREEN_MAX_WIDTH_3D/(TOUCH_MAX_HEIGHT_EDT_2S);
  				input_x = SCREEN_MAX_HEIGHT_3D-input_x *SCREEN_MAX_HEIGHT_3D/(TOUCH_MAX_WIDTH_EDT_2S);	
  				input_y = input_y *SCREEN_MAX_WIDTH_3D/(TOUCH_MAX_HEIGHT_EDT_2S);
					tmp = input_x;
					input_x = input_y;
					input_y = tmp;
				...
				{
					input_x = input_x *1920/(2496);	
					input_y = input_y *1200/(1536);
					
				}
				else if(machine_is_t8400n()|| machine_is_keenhi01n())
				{

					//input_x =1920-input_x *1920/(2816);//1280 1792
					//input_y =input_y *1200/(1792);//800 980
#if defined(CONFIG_MACH_T8400N_8_3CM)
					input_x =input_x *1920/1920;//(2773);//1280 1792
					input_y =1200-input_y *1200/1200;//(1685);//800 980
					tmp = input_x;
					...
					input_y = tmp;
#else
					input_x =input_x *1920/(3328);//1280 1792
					input_y =input_y *1080/(1856);//800 980
#endif
			...
			case JIN_TOUCH :
				...
				break;
			case RTR_TOUCH:
				if(machine_is_nabi2_3d() || machine_is_qc750() ||  machine_is_n750() ||  machine_is_itq700() || machine_is_itq701()  || machine_is_wikipad())
				{
					input_x = input_x *SCREEN_MAX_HEIGHT_3D/(1660);	
					input_y = input_y *SCREEN_MAX_WIDTH_3D/(1000);
					tmp = input_x;
					input_x = input_y;
					input_y = 1280- tmp;
				}
			...
			//printk("input_x = %d,input_y = %d, track_id[index] = %d\n", input_x, input_y, track_id[index]);
...
static irqreturn_t novatek_ts_irq_handler(int irq, void *dev_id)
{
	...
	int level  = 0;
	level = gpio_get_value(ts->discoveryed_pencil_gpio);

#if defined(CONFIG_MACH_T8400N_8_3CM)
	if(level) return IRQ_HANDLED;
#endif
...
static void Novatek_tp_power_supply(bool state)//true:vdd power on,false:vdd power off
{
  int ret=0;
  if(machine_is_terra10()||machine_is_terra7()||machine_is_t8400n()|| machine_is_keenhi01n())
  {
...
void set_touch_pannel_charge_in(int status)
{
	uint8_t data1[] = {0xff, 0x8f, 0xff};
	uint8_t cmd1[] = {0x00, 0xC7};
	uint8_t cmd2[] = {0x00, 0xC8};
	uint8_t test_data[7] = {0x00,};
	if(!i2c_connect_client_novatek)
		return;
	if(i2c_read_bytes(i2c_connect_client_novatek, test_data, 5)<=0)//test i2c
		{
		return;
		}
	if(machine_is_terra10())
	{
		if(touch_type!=EDT_TOUCH)
			return;
		if(status == 1)	// 0xC7 charger plug in
		{
			//printk("harge plug in,write i2c data\n");
			i2c_write_bytes(i2c_connect_client_novatek, data1, 3);
			i2c_write_bytes(i2c_connect_client_novatek, cmd1, 2);
		}
		else	if(status==0)// 0xC8 charger plug out
		{
			//printk("harge plug out,write i2c data\n");
			i2c_write_bytes(i2c_connect_client_novatek, data1, 3);
			i2c_write_bytes(i2c_connect_client_novatek, cmd2, 2);
		}
	}
}
EXPORT_SYMBOL_GPL(set_touch_pannel_charge_in);

static int	nvctp_CheckIsBootloader(struct i2c_client *client);
static int Novatek_tp_enable(struct input_dev *input_dev);
static int Novatek_tp_disable(struct input_dev *input_dev);
#ifdef STYLUS_SUPPORT
#define NORMAL_MODE 0
#define STYLUS_MODE 1
static int stylus_status=NORMAL_MODE;

void novatek_stylus_mode_change(int status)
{
	uint8_t data1[] = {0xff, 0x8f, 0xff};
	uint8_t cmd1[] = {0x00, 0xD1};//stylus mode
	uint8_t cmd2[] = {0x00, 0xD2};//normal mode
	if(!i2c_connect_client_novatek)
		return;
...
static ssize_t novatek_tp_show(struct device *dev,
				     struct device_attribute *attr, char *buf)
{
...
static ssize_t novatek_tp_store(struct device *dev,
						...
						const char *buf, size_t count)
{
	...
	if(buf[0]=='0')
	{
		...
		novatek_stylus_mode_change(NORMAL_MODE);
		stylus_status=NORMAL_MODE;
		
	}
	else if(buf[0]=='1')
	{
		printk("enter stylus mode\n");
		novatek_stylus_mode_change(STYLUS_MODE);
		stylus_status=STYLUS_MODE;
...
static struct device_attribute nova_ts_attributes[] = {
	__ATTR(tp_control, 0644, novatek_tp_show, novatek_tp_store),
	__ATTR_NULL,
};
#endif
				     ...
				     struct device_attribute *attr, char *buf)
{
	...
	{
		ret =i2c_read_bytes(i2c_connect_client_novatek, test_data, 5);
		if (ret > 0){
			break;
		}
...
static int novatek_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	struct novatek_i2c_platform_data *pdata;
...
//	input_set_abs_params(ts->input_dev, ABS_Y, 0, ts->abs_y_max-1, 0, 0);
	...
	ts->input_dev->enable=Novatek_tp_enable;
	ts->input_dev->disable=Novatek_tp_disable;
#ifdef STYLUS_SUPPORT
	device_create_file(&ts->client->dev,&nova_ts_attributes);
#endif
...
#endif
	if(machine_is_nabi2_xd() || machine_is_nabi_2s() || machine_is_n1010()||machine_is_ns_14t004()||machine_is_itq100()||
		machine_is_n1011() || machine_is_n1050()||machine_is_terra10()||machine_is_terra7()||machine_is_t8400n()|| machine_is_keenhi01n()){
		novatek_reset_int();
	}
	return 0;
}
...
{
	struct novatek_ts_data *ts;
    	//ts = container_of(input_dev, struct novatek_ts_data, input_dev);
	printk("novatek_tp_will_resume!\n");
	Novatek_tp_power_supply(true);
	novatek_ts_resume(i2c_connect_client_novatek);
#ifdef STYLUS_SUPPORT
	msleep(100);
	if(stylus_status==STYLUS_MODE)//tp will change to normal mode after waking up
		novatek_stylus_mode_change(STYLUS_MODE);
	else
		novatek_stylus_mode_change(NORMAL_MODE);
#endif
...
static int Novatek_tp_disable(struct input_dev *input_dev)
{
	struct novatek_ts_data *ts;
    	//ts = container_of(input_dev, struct novatek_ts_data, input_dev);
	printk("novatek_tp_will_suspend!\n");
	novatek_ts_suspend(i2c_connect_client_novatek, PMSG_SUSPEND);
	Novatek_tp_power_supply(false);
	return 1;
...
#if defined(CONFIG_MACH_TERRA10) || defined CONFIG_MACH_T8400N || defined(CONFIG_MACH_KEENHI01N)
#define  FW_DATASIZE_NEW      (1024*28)
#else
...
static int novatek_FlashEraseMass(struct i2c_client *client)
{
	...
	wAddress = 0;
	for(i=0;i<FLASHSECTORSIZE_NEW;i++)
	{	
 ...
 switch(touch_type)
 {
	case EDT_TOUCH:
		if(machine_is_wikipad())
			...
			FW_CHECKSUM = (unsigned int)(nvctp_BinaryFile_edt_terra10[FW_DATASIZE-2]<< 8 |nvctp_BinaryFile_edt_terra10[FW_DATASIZE-1]);
			}
		else if(machine_is_t8400n()|| machine_is_keenhi01n()){
			FW_CHECKSUM = (unsigned int)(nvctp_BinaryFile_edt_t8400n[FW_DATASIZE-2]<< 8 |nvctp_BinaryFile_edt_t8400n[FW_DATASIZE-1]);
			}
		else
			FW_CHECKSUM = (unsigned int)(nvctp_BinaryFile_edt[FW_DATASIZE-2]<< 8 |nvctp_BinaryFile_edt[FW_DATASIZE-1]);
		break;
	case WISTRON_TOUCH:
		FW_CHECKSUM = (unsigned int)(nvctp_BinaryFile_wistron_t8400n[FW_DATASIZE-2]<< 8 |nvctp_BinaryFile_wistron_t8400n[FW_DATASIZE-1]);
		break;
	case TTL_TOUCH:
		FW_CHECKSUM = (unsigned int)(nvctp_BinaryFile_ttl_t8400n[FW_DATASIZE-2]<< 8 |nvctp_BinaryFile_ttl_t8400n[FW_DATASIZE-1]);
		break;
	case RACEE_TOUCH:
		if(machine_is_terra10())
			FW_CHECKSUM = (unsigned int)(nvctp_BinaryFile_racee_terra10[FW_DATASIZE-2]<< 8 |nvctp_BinaryFile_racee_terra10[FW_DATASIZE-1]);
		break;
	case CNT_TOUCH:
	...
	case JIN_TOUCH :
		...
		break;
	case RTR_TOUCH:
		if(machine_is_nabi_2s() || machine_is_qc750() ||  machine_is_n750() ||  machine_is_itq700() || machine_is_itq701())
	...
	NOVADBG("nvctp_CheckIsBootloader version = %x,%x,%x,%x\n",iic_data_buffer[1],iic_data_buffer[0],iic_data_buffer[2],write_cmd1[1]);
{
		...
		if(pdata->gpio_discovery_pencil>0){
			ts->discoveryed_pencil_gpio = pdata->gpio_discovery_pencil;
		}
		if(pdata->detect)
			touch_type = pdata->detect();
		if(pdata->detect1)
			touch_type1 = pdata->detect1();
		printk("detect touch type = %d\n",touch_type);
		printk("detect touch type1 = %d\n",touch_type1);
		
		if(machine_is_nabi2_3d()||machine_is_qc750()  || machine_is_itq700()  || machine_is_itq701())
		...
		if( machine_is_n1010()||machine_is_ns_14t004()||machine_is_itq100()||machine_is_n1011()|| machine_is_n1050()){
		...
		if(machine_is_terra10())
		{
			if(touch_type == 0x0e)
				touch_type = EDT_TOUCH;
			else if(touch_type == 0x0a)
				touch_type = RACEE_TOUCH;
			else return;
		}
		if(machine_is_t8400n()|| machine_is_keenhi01n())
		{
			if(touch_type1==0x01)
				touch_type = EDT_TOUCH;
			else if(touch_type1==0x02)
				touch_type = QTW_TOUCH;
			else if(touch_type1==0x03)
				{
				touch_type = WISTRON_TOUCH;//for wistrom tp
				}
			else if(touch_type1==0x00)
				{
				touch_type= TTL_TOUCH;
				}
		}
		...
		switch(touch_type)
		{
			...
			case JIN_TOUCH :
				...
				break;
			case RTR_TOUCH:
				if(machine_is_qc750() ||  machine_is_n750() ||  machine_is_itq700() || machine_is_itq701())
					FW_CHECKSUM = (unsigned int)(nvctp_BinaryFile_rtr_n710[FW_DATASIZE-2]<< 8 |nvctp_BinaryFile_rtr_n710[FW_DATASIZE-1]);
				else if(machine_is_wikipad()) {
					FW_CHECKSUM = (unsigned int)(nvctp_BinaryFile_rtr_wikipad[FW_DATASIZE-2]<< 8 |nvctp_BinaryFile_rtr_wikipad[FW_DATASIZE-1]);
				}
		...
		switch(touch_type)
		{
			case EDT_TOUCH:
				if(machine_is_wikipad())
					...
					novatek_Bootloader(nvctp_BinaryFile_edt_terra10,sizeof(nvctp_BinaryFile_edt_terra10),ts->client);
					}
				else if(machine_is_t8400n()|| machine_is_keenhi01n()){
					novatek_Bootloader(nvctp_BinaryFile_edt_t8400n,sizeof(nvctp_BinaryFile_edt_t8400n),ts->client);
					}
				else	
					novatek_Bootloader(nvctp_BinaryFile_edt,sizeof(nvctp_BinaryFile_edt),ts->client);
				break;
			case WISTRON_TOUCH:
				novatek_Bootloader(nvctp_BinaryFile_wistron_t8400n,sizeof(nvctp_BinaryFile_wistron_t8400n),ts->client);
				break;
			case TTL_TOUCH:
				novatek_Bootloader(nvctp_BinaryFile_ttl_t8400n,sizeof(nvctp_BinaryFile_ttl_t8400n),ts->client);
				break;
			case RACEE_TOUCH:
				if(machine_is_terra10())
					novatek_Bootloader(nvctp_BinaryFile_racee_terra10,sizeof(nvctp_BinaryFile_racee_terra10),ts->client);
				break;
			case CNT_TOUCH:
