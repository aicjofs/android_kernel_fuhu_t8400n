...
static void dw9718_get_focuser_capabilities(struct dw9718_info *info)
{
	...
	info->nv_config.range_ends_reversed = info->cap.range_ends_reversed;

	info->nv_config.pos_working_low = info->cap.pos_working_low;
	info->nv_config.pos_working_high = info->cap.pos_working_high;
	info->nv_config.pos_actual_low = info->cap.pos_actual_low;
	info->nv_config.pos_actual_high = info->cap.pos_actual_high;

	info->nv_config.slew_rate = info->cap.slew_rate;
	info->nv_config.circle_of_confusion = info->cap.circle_of_confusion;
	info->nv_config.num_focuser_sets = info->cap.num_focuser_sets;

	for (i = 0; i < info->nv_config.num_focuser_sets; i++) {
		info->nv_config.focuser_set[i].posture =
							info->cap.focuser_set[i].posture;
		info->nv_config.focuser_set[i].macro =
							info->cap.focuser_set[i].macro;
		info->nv_config.focuser_set[i].hyper =
							info->cap.focuser_set[i].hyper;
		info->nv_config.focuser_set[i].inf =
							info->cap.focuser_set[i].inf;
		info->nv_config.focuser_set[i].hysteresis =
							info->cap.focuser_set[i].hysteresis;
		info->nv_config.focuser_set[i].settle_time =
							info->cap.focuser_set[i].settle_time;
		info->nv_config.focuser_set[i].macro_offset =
							info->cap.focuser_set[i].macro_offset;
		info->nv_config.focuser_set[i].inf_offset =
							info->cap.focuser_set[i].inf_offset;
		info->nv_config.focuser_set[i].num_dist_pairs =
							info->cap.focuser_set[i].num_dist_pairs;

		for ( j = 0; j < info->nv_config.focuser_set[i].num_dist_pairs; j++) {
			info->nv_config.focuser_set[i].dist_pair[j].fdn =
							info->cap.focuser_set[i].dist_pair[j].fdn;
			info->nv_config.focuser_set[i].dist_pair[j].distance =
							info->cap.focuser_set[i].dist_pair[j].distance;
		}
	}
}
					...
					struct nvc_param *params)
{
	...
	info->cap.range_ends_reversed = info->nv_config.range_ends_reversed;

	info->cap.pos_working_low = info->nv_config.pos_working_low;
	info->cap.pos_working_high = info->nv_config.pos_working_high;

	if (info->nv_config.pos_actual_low < 0 ||
			info->nv_config.pos_actual_high > 1023)
		info->cap.pos_actual_low = dw9718_POS_LOW_DEFAULT ;
	else
		info->cap.pos_actual_low = info->nv_config.pos_actual_low;

	if (info->nv_config.pos_actual_high <0 ||
			info->nv_config.pos_actual_high > 1023)
		info->cap.pos_actual_high = dw9718_POS_HIGH_DEFAULT;
	else
		info->cap.pos_actual_high = info->nv_config.pos_actual_high;

	if (info->pdata->cap->slew_rate)
		info->cap.slew_rate = info->pdata->cap->slew_rate;
	else
		info->cap.slew_rate = info->nv_config.slew_rate;

	info->cap.circle_of_confusion = info->nv_config.circle_of_confusion;
	info->cap.num_focuser_sets = info->nv_config.num_focuser_sets;

	for (i = 0; i < info->cap.num_focuser_sets; i++) {
		info->cap.focuser_set[i].posture =
							info->nv_config.focuser_set[i].posture;
		info->cap.focuser_set[i].macro =
							info->nv_config.focuser_set[i].macro;
		...
		for ( j = 0; j < info->cap.focuser_set[i].num_dist_pairs; j++) {
			info->cap.focuser_set[i].dist_pair[j].fdn =
							info->nv_config.focuser_set[i].dist_pair[j].fdn;
			info->cap.focuser_set[i].dist_pair[j].distance =
							info->nv_config.focuser_set[i].dist_pair[j].distance;
