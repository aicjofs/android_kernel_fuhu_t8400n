...
#define SENSOR_IOCTL_SET_OP_MODE		_IOW('o', 0xa, KhOperationalMode)
enum {
      YUV_ColorEffect = 0,
      YUV_Whitebalance,
      YUV_SceneMode
...
struct sensor_info {
	...
	struct yuv_hm2056_sensor_platform_data *pdata;
};

static struct sensor_info *hm2056_sensor_info;
...
{0x0128,0x1F},  //                               
{0x0132,0x10},  //                               
{0x0131,0xBD},  // simle bpc enable[4]    20130128         20140514  AD->BD
{0x0140,0x14},  //                               
{0x0141,0x0A},  //                               
{0x0142,0x14},  //                               
{0x0143,0x0A},  //                               
{0x0144,0x06},  // Sort bpc hot pixel ratio      20140514 04->06 
{0x0145,0x00},  //                               
{0x0146,0x20},  //                               
{0x0147,0x0A},  //                               
{0x0148,0x10},  //                               
...
{0x0380,0xFF},  //                               
{0x0381,0x4A},  //                               
{0x0382,0x36},  //     20130923--36-->56                          
{0x038A,0x42},  //     20130923--40-->42                          
{0x038B,0x08},  //                               
{0x038C,0xC1},  //                               
{0x038E,0x40},  //20130923-- 40->4c                           
{0x038F,0x04},  //                               
{0x0390,0x8c},  //                               
{0x0391,0x05},  //                               
{0x0393,0x80},  //                               
...
{0x0480,0x5f},  //  64->67   QPS 67->58->47 >50->4b ->4e 4a                           
{0x0481,0x06},  //                               
{0x0482,0x0C},  //                               
...
//    {0x007C,0x33,0,100},  // pre-hsync setting , 2013-01-17
...
static struct sensor_reg mode_320x240[] = {
{0x0005,0x00},
{0x011F,0xF7},	//
{0x0125,0xFF},	//
{0x0126,0x70},	//
...
static struct sensor_reg mode_352x288[] = {
		 //CIF(352 x 288 ) //
 {0x05E1,0x00},
 {0x05E2,0x06},
 {0x05E3,0x01},
...
{0x0380,0xFE},	// disable AE : [0] = 0
{0x0015,0x01},	//
{0x0016,0x87},	// 48M
{0x0006,0x00},	//
{0x000D,0x11},	//
{0x000E,0x11},	//
{0x000F,0x00},	// ZXW 20121116 fix fps //o 0x00
{0x0011,0x02},	//
{0x0012,0x1C},	// 2012.02.08
{0x0013,0x01},	//
{0x0026,0x87},	// 48M
...
{0x002A,0x1F},	// PLL Setting For 30fps under VGA @ 24MHz MCLK, MIPI 384bps
{0x0070,0x5F},	//
{0x0071,0xFF},	//
{0x0072,0x55},	//
{0x0073,0x50},	//
...
{0x038E,0x32},//o 0x4c
{0x04B0,0x58},//o 0x64
{0x0480,0x5f}, //Saturation ,40->47->50 , 03-01	20140303
{0x04C0,0x00},
{0x0481,0x08},
{0x02E0,0x07},
{0x038F,0x03},	// Max INTG// zxw for 20-30fps
{0x0390,0xAE},	// 
{0x05E0,0xA0},	// Scaler
{0x05E1,0x00},	//
{0x05E2,0xA0},	//
...
{0x000D,0x00},	//
{0x000E,0x00},	//
{0x000F,0x10},	// ZXW 20121116 fix fps //o 0x00
{0x0011,0x02},	//
{0x0012,0x08},	//
{0x0013,0x00},	//
{0x0026,0x87},	// 0X88 64M, 0X87 72M
...
//{0x038F,0x02},	// Max INTG
//{0x0390,0xf0},	//
{0x0020,0x00},	// Max INTG
{0x038F,0x03},	// Max INTG
{0x0390,0x84},	//
{0x03BB,0x9F},  // enable 5x5 window
{0x03BC,0xCF},
{0x03BD,0xE7},
...
static struct sensor_reg mode_1280x752[] = {
{0x0005,0x00},
{0x0024,0x00},
{0x0006,0x04},
{0x000D,0x00},
...
static struct sensor_reg mode_1440x1080[] = {

{0x0005,0x00},  // Rolling Shutter Off
{0x0006,0x00},  //
{0x000D,0x00},  //
{0x000E,0x00},  //
{0x000F,0x10}, // zxw 20121116 dynamic //o 0x00
{0x0011,0x09},  //
{0x0012,0x02},  //
{0x0013,0x09},  //
{0x002A,0x2F},  // PLL Setting For 15fps under VGA @ 24MHz MCLK, MIPI 576bps
...
{0x0190,0x87},  // A11 BPC Strength[7:3], cluster correct P11[0]P12[1]P13[2]
{0x0192,0x50},  // A13 Strength[4:0],hot pixel detect for cluster[6]
{0x038F,0x06},  // Max INTG //zxw 20121116 ,10 -15fps 
{0x0390,0xcc},  //
{0x0541,0x80},  // 60Hz Flicker
{0x0543,0x9A},  // 50Hz Flicker
...
static struct sensor_reg *mode_table[] = {
	...
	[SENSOR_MODE_1280x752]  = mode_1280x752,
	[SENSOR_MODE_1280x960]  = mode_1280x960,
	[SENSOR_MODE_1440x1080]  = mode_1440x1080,
...
static int sensor_write_reg8_addr8(struct i2c_client *client, u8 addr, u8 val)
{
	...
	do {
		...
		if (err == 1)
			return 0;
		retry++;
		pr_err("hm2056 : i2c transfer failed, retrying %x %x %d\n",
		       addr, val, err);
		msleep(3);
	} while (retry <= SENSOR_MAX_RETRIES);
			      ...
			      const struct sensor_reg table[])
{
	...
	for (next = table; next->addr!= SENSOR_TABLE_END; next++) {
		switch (next->addr) {
			case SENSOR_WAIT_MS:
			{
				pr_err("sensor_write_table:wait 0x%xms ...",next->val);
				msleep(next->val);
				continue;
			}
			//default:break;
		...
		data = next->val;
	       ...
	       err = sensor_write_reg8_addr16(client, next->addr,next->val);
		if (err){
...
static int sensor_set_mode(struct sensor_info *info, struct sensor_mode *mode)
{
	...
	if (mode->xres == 320 && mode->yres == 240)
		sensor_table = SENSOR_MODE_320x240;
	else if (mode->xres == 352 && mode->yres == 288)
		sensor_table = SENSOR_MODE_352x288;
	...
	else if(mode->xres == 1280 && mode->yres == 752)//1280x752
		sensor_table = SENSOR_MODE_1280x752;
	else if(mode->xres == 1280 && mode->yres == 960)//1280x720
		sensor_table = SENSOR_MODE_1280x960;
	else if(mode->xres == 1440 && mode->yres == 1080)//1440x1080
		sensor_table = SENSOR_MODE_1440x1080;
			 ...
			 unsigned int cmd, unsigned long arg)
{
	...
	case SENSOR_IOCTL_SET_OP_MODE:
        ...
        {
            u8 whitebalance;
		debug_print("######################ioctl YUV_white balance EXE 1!!!!!!!!\n");
	 ...
	 case SENSOR_IOCTL_SET_YUV_EXPOSURE:
        {
	      u8 yuvexposure;
		debug_print("######################ioctl YUV_exposure EXE 1!!!!!!!!\n");
	 ...
	 case SENSOR_IOCTL_SET_SCENE_MODE:
        {
              u8 scene_mode;
		debug_print("######################ioctl scene mode EXE !!!!!!!!\n");
...
{
	debug_print("hm2057 %s\n",__func__);
	file->private_data =hm2056_sensor_info;
	if (hm2056_sensor_info->pdata &&hm2056_sensor_info->pdata->power_on){
		hm2056_sensor_info->pdata->power_on();
		atomic_set(&hm2056_sensor_info->power_on_loaded, 1);
		atomic_set(&hm2056_sensor_info->sensor_init_loaded, 0);
	}
	return 0;
}
...
{
	debug_print("hm2057 %s\n",__func__);
	if (hm2056_sensor_info->pdata &&hm2056_sensor_info->pdata->power_off){
		hm2056_sensor_info->pdata->power_off();
		atomic_set(&hm2056_sensor_info->power_on_loaded, 0);
		atomic_set(&hm2056_sensor_info->sensor_init_loaded, 0);
	}
...
{
    struct sensor_info *info =hm2056_sensor_info;
    ssize_t res = 0;
...
#endif
static DEVICE_ATTR(hm_camera_effect, S_IRUSR|S_IWUSR, hm_camera_effect_show,hm_camera_effect_store);
			...
			const struct i2c_device_id *id)
{
	...
	hm2056_sensor_info = kzalloc(sizeof(struct sensor_info), GFP_KERNEL);
	if (!hm2056_sensor_info) {
	...
	hm2056_sensor_info->pdata = client->dev.platform_data;
	hm2056_sensor_info->i2c_client = client;
	atomic_set(&hm2056_sensor_info->power_on_loaded, 0);
	atomic_set(&hm2056_sensor_info->sensor_init_loaded, 0);
	
	sensor_t8ev5_dev = platform_device_register_simple("hm2056", -1, NULL, 0);
	...
	mutex_init(&hm2056_sensor_info->als_get_sensor_value_mutex);
	i2c_set_clientdata(client,hm2056_sensor_info);
	pr_info("hm2057 %s register successfully!\n",__func__);
	return 0;
EXIT:
	kfree(hm2056_sensor_info);
	return err;
}
