...
#define CAM1_POWER_DWN_GPIO 221 /* TEGRA_GPIO_PBB5 */
...
static void ar0833_gpio_init( struct ar0833_info *info)
{	
	int err;	
	err = gpio_request(CAM1_POWER_DWN_GPIO, "camera-pwdn");	
	if (err)		
		dev_err(&info->i2c_client->dev,				
		"%s: gpio camera-pwdn request faild", __func__);
