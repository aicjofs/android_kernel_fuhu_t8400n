...
static u8 check_sensor_type(struct sensor_info *info){
	...
	if(info->sensor_type == SENSOR_T8EV5||info->sensor_type == SENSOR_MT9P11) {
		debug_print ("%s:======>it have checked sensor so skip.\n",__func__);
...
static int sensor_open(struct inode *inode, struct file *file)
{
	...
	if (mt9p111_sensor_info->pdata && mt9p111_sensor_info->pdata->power_on){
		...
		if(sensor_type == SENSOR_T8EV5){
			goto exit;
		}else
			mt9p111_sensor_info->pdata->power_off();
	}
	if (mt9p111_sensor_info->pdata && mt9p111_sensor_info->pdata->second_power_on){
		mt9p111_sensor_info->pdata->second_power_on();
		sensor_type = check_sensor_type(mt9p111_sensor_info);
		if(sensor_type == SENSOR_MT9P11){
			goto exit;
		}else{
			mt9p111_sensor_info->pdata->second_power_off();
			}
	}
exit:
	debug_print("%s: sensor type= %d\n",__func__,sensor_type);
	atomic_set(&mt9p111_sensor_info->power_on_loaded, (sensor_type==SENSOR_MT9P11||sensor_type==SENSOR_T8EV5));
...
{
if(mt9p111_sensor_info->sensor_type == SENSOR_T8EV5){
...
else if(mt9p111_sensor_info->sensor_type == SENSOR_MT9P11){

	if (mt9p111_sensor_info->pdata && mt9p111_sensor_info->pdata->second_power_off)
		mt9p111_sensor_info->pdata->second_power_off();
}
	
	atomic_set(&mt9p111_sensor_info->power_on_loaded, 0);
	atomic_set(&mt9p111_sensor_info->sensor_init_loaded, 0);
	//mt9p111_sensor_info->sensor_type = SENSOR_Invalid;
...
static int set_sensor_effect(struct sensor_info *info){
	int err = 0;
	if(info->current_wb>YUV_Whitebalance_Invalid&&info->current_wb<YUV_Whitebalance_Shade){
		if(info->sensor_type == SENSOR_T8EV5)
	...
	if(info->current_exposure>YUV_YUVExposure_Invalid&&info->current_exposure<=YUV_YUVExposure_Negative2){
		if(info->sensor_type == SENSOR_T8EV5)
...
static int sensor_set_mode(struct sensor_info *info, struct sensor_mode *mode)
{
	...
	if(atomic_read(&info->power_on_loaded)&&!atomic_read(&info->sensor_init_loaded)){
		...
		debug_print("%s:sensor camera init setting reg inited!\n",__func__);
			...
			const struct i2c_device_id *id)
{
	...
	if(mt9p111_sensor_info->pdata){
		...
		if(mt9p111_sensor_info->timer_debounce)
