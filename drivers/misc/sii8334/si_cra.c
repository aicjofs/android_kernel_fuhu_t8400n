...
uint8_t SiiRegRead ( SiiReg_t virtualAddr )
{
	...
	////TX_I2C_DEBUG_PRINT(("SiiRegRead:virtualAddr start = 0x%04x regOffset=%02x,value=%02x!\n",virtualAddr,regOffset,value));
    virtualAddr >>= 8;
	////TX_I2C_DEBUG_PRINT(("SiiRegRead:virtualAddr end = %d !\n",virtualAddr));
...
void SiiRegWrite ( SiiReg_t virtualAddr, uint8_t value )
{
    uint8_t             regOffset = (uint8_t)virtualAddr;
    pageConfig_t        *pPage;
	//TX_I2C_DEBUG_PRINT(("SiiRegWrite:virtualAddr start = 0x%04x regOffset=%02x,value=%02x!\n",virtualAddr,regOffset,value));
    virtualAddr >>= 8;
	//TX_I2C_DEBUG_PRINT(("SiiRegWrite:virtualAddr end = %d !\n",virtualAddr));
    pPage = &g_addrDescriptor[l_pageInstance[virtualAddr]][virtualAddr];
...
void    SiiRegReadBlock ( SiiReg_t virtualAddr, uint8_t *pBuffer, uint16_t count )
{
	...
	//TX_I2C_DEBUG_PRINT(("SiiRegReadBlock:virtualAddr start = 0x%04x regOffset=%02x!\n",virtualAddr,regOffset));
	virtualAddr >>= 8;
	//TX_I2C_DEBUG_PRINT(("SiiRegReadBlock:virtualAddr end = %d !\n",virtualAddr));
	pPage = &g_addrDescriptor[l_pageInstance[virtualAddr]][virtualAddr];
	switch(pPage->address)
	   {
	   case DEV_PAGE_TPI_0:
...
void SiiRegEdidReadBlock ( SiiReg_t segmentAddr, SiiReg_t virtualAddr, uint8_t *pBuffer, uint16_t count )
{
	...
	//TX_I2C_DEBUG_PRINT(("SiiRegEdidReadBlock:virtualAddr start = 0x%04x regOffset=%02x!\n",virtualAddr,regOffset));
    // Read the actual EDID data
    regOffset = (uint8_t)virtualAddr;
    virtualAddr >>= 8;
    //TX_I2C_DEBUG_PRINT(("SiiRegEdidReadBlock:virtualAddr end = %d !\n",virtualAddr));
    pPage = &g_addrDescriptor[l_pageInstance[virtualAddr]][virtualAddr];
...
void    SiiRegWriteBlock ( SiiReg_t virtualAddr, const uint8_t *pBuffer, uint16_t count )
{
	...
	//TX_I2C_DEBUG_PRINT(("SiiRegWriteBlock:virtualAddr start = 0x%04x regOffset=%02x,value=%02x!\n",virtualAddr,regOffset));
	virtualAddr >>= 8;
	//TX_I2C_DEBUG_PRINT(("SiiRegWriteBlock:virtualAddr end = %d !\n",virtualAddr));
	pPage = &g_addrDescriptor[l_pageInstance[virtualAddr]][virtualAddr];
	switch(pPage->address)
	   {
	   case DEV_PAGE_TPI_0:
