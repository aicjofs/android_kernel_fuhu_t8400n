...
static struct mhl_sii8334_platform_data *Sii8334_plat_data;
...
static int Sii8334_regulator_get(struct i2c_client *client)
{
	struct regulator *reg = NULL;
	char *reg_name = "mhl_1v2";
	int err = 0;
	pr_err("%s:==========>entried!\n",__func__);
	reg = regulator_get(NULL, reg_name);
	if (unlikely(IS_ERR(reg))) {
		dev_err(&client->dev, "%s %s ERR: %d\n",
			__func__, reg_name, (int)reg);
		err = PTR_ERR(reg);
		reg = NULL;
	} else {
		pr_err("%s: %s\n",
		...
		if (unlikely(err))
			dev_err(&client->dev, "%s: enable %s fail!\n",
			__func__, reg_name);
		else
			regulator_set_voltage(reg, 1200000, 1200000);
		//pr_err("%s:==========>get regulater success!\n",__func__);
	}
			       ...
			       unsigned int cmd, unsigned long arg)
{
	 ...
	 * these IOCTL.
	 */
	case TEGRA_MHL_IOCTL_CMDS:
	/* first element of arg must be u32 with id of module to talk to */
	if (copy_from_user(&cmds, (const void __user *)arg, sizeof(uint))) {
		pr_err("%s: Failed to copy arg from user", __func__);
		return -EFAULT;
	}

	if (cmds >= MHL_RCP_CMD_RSVD) {
		dev_err("%s: Invalid id to sii8334 ioctl%d\n",
				__func__, cmds);
		return -EINVAL;
	}
	pr_err("%s:============>cmds = %d\n",__func__,cmds);
	input_report_mhl_rcp_key(cmds);
	default:
		dev_err("%s: Unknown tegra_camera ioctl.\n", __func__);
		return -EINVAL;
	}
	return 0;
}
...
static const struct file_operations tegra_mhl_fops = {
	.owner = THIS_MODULE,
...
static struct miscdevice misc_devs = {
			...
			const struct i2c_device_id *dev_id)
{
	...
	*/
		...
		&& sii8334_PAGE_CBUS != NULL)
	{
		...
		SiiMhlTxInitialize();
		misc_devs.fops = &tegra_mhl_fops;
		ret = misc_register(&misc_devs);
