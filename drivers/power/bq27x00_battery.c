				...
				int chrg_type)
{
	...
	if (status == bq2419_progress||status == bq2419_completed) {
		g_charge_status = POWER_SUPPLY_STATUS_CHARGING;
		if(g_soc>=100)
			g_charge_status = POWER_SUPPLY_STATUS_FULL;	
		}
	else
		{
			g_charge_status = POWER_SUPPLY_STATUS_DISCHARGING;
		}
...
static void battery_status_poll(struct work_struct *work)
{
	...
	bq27541_get_psp(POWER_SUPPLY_PROP_TEMP);
	bq27541_get_psp(POWER_SUPPLY_PROP_VOLTAGE_NOW);	
...
static int bq27541_get_capacity()
{
	...
	do {
		bq27541_device->smbus_status = bq27541_smbus_read_data(REG_CAPACITY, 0 ,&tmp);
	...
	g_soc =bq27541_device->bat_capacity = temp_capacity;
	bq27541_device->cap_err=0;

	BAT_NOTICE("= %u%% ret= %u\n", bq27541_device->bat_capacity, ret);
	return 0;
}
	...
	const struct i2c_device_id *id)
{
	...
	bq27541_read_i2c(BQ27500_REG_SOC, &rsoc, 0);
	mdelay(500);
	bq27541_read_i2c(BQ27x00_REG_VOLT, &volt, 0);
	mdelay(500);
