...
#define  DEFAULT_ADC     0
#define  TERRA7_ADC      1
#define  TERRA10_ADC     2
...
static struct rt5639_init_reg init_list[] = {
	...
	{RT5639_HP_VOL		, 0x8a8a},/* OUTMIX -> HPVOL */
...
#if (CONFIG_TEGRA_RT5639_ADC==TERRA10_ADC)
	{RT5639_SPK_VOL		, 0x8484},/* SPKMIX -> SPKVOL */
	{RT5639_SPO_CLSD_RATIO	, 0x0007},
#elif(CONFIG_TEGRA_RT5639_ADC==TERRA7_ADC)
    {RT5639_SPK_VOL		, 0x8484},/* SPKMIX -> SPKVOL */
    {RT5639_SPO_CLSD_RATIO	, 0x0005},
#else
    {RT5639_SPK_VOL		, 0x8484},/* SPKMIX -> SPKVOL */
	...
	/*record*/
#if (CONFIG_TEGRA_RT5639_ADC==TERRA10_ADC)
	{RT5639_IN1_IN2		, 0x8080},/* IN1 boost 40db and
					     differential mode */
	{RT5639_IN3_IN4		, 0x0800},/* IN2 boost 40db and
					     signal ended mode */
#elif (CONFIG_TEGRA_RT5639_ADC==TERRA7_ADC)
    {RT5639_IN1_IN2		, 0x7080},/* IN1 boost 40db and
					     differential mode */
...
#if (CONFIG_TEGRA_RT5639_ADC==TERRA10_ADC)
	{RT5639_DRC_AGC_1	 ,0xe020},
	{RT5639_DRC_AGC_2	 ,0x3f80},
	{RT5639_DRC_AGC_3    ,0x2060},
#endif
};
#define RT5639_INIT_REG_LEN ARRAY_SIZE(init_list)


#define RT5639_INIT_PHONE_REG_LEN ARRAY_SIZE(init_list_phone)
static struct rt5639_init_reg init_list_phone[]={
	{RT5639_PWR_ANLG1	,0xA810},
	
	{RT5639_REC_R1_MIXER	,0x3600},
	{0xfa	,0x3f01},
	{RT5639_PWR_ANLG1	,0xE8FC},
	{RT5639_PWR_DIG1	,0xD807},
	{RT5639_PWR_DIG2	,0xE000},
	{RT5639_PWR_MIXER	,0xFC00},
	{RT5639_PWR_VOL	,0xFC00},
	{RT5639_PWR_ANLG2	,0x9A00},
	{RT5639_STO_ADC_MIXER	, 0x3020},/* ADC -> Sto ADC mixer */ //3020
	{RT5639_REC_L2_MIXER,0x007d},
	{RT5639_REC_R1_MIXER,0x0000},
	{RT5639_REC_R2_MIXER,0x007d},
};

static int rt5639_reg_phone_init(struct snd_soc_codec *codec)
{
	int i;

	for (i = 0; i < RT5639_INIT_PHONE_REG_LEN; i++)
		snd_soc_write(codec, init_list_phone[i].reg, init_list_phone[i].val);
...
#if(CONFIG_TEGRA_RT5639_ADC==TERRA7_ADC)
	[RT5639_ADC_DIG_VOL] = 0x5f5f,                   //0x4343,
	[RT5639_ADC_DATA] =  0x5f5f,                          //0x4343,
#elif(CONFIG_TEGRA_RT5639_ADC==TERRA10_ADC)
	[RT5639_ADC_DIG_VOL] = 0x5f5f,
	[RT5639_ADC_DATA] = 0x5f5f,
#else 
	[RT5639_ADC_DIG_VOL] = 0x5f5f,
	[RT5639_ADC_DATA] = 0x5f5f,
#endif
...
int rt5639_headset_detect(struct snd_soc_codec *codec, int jack_insert)
{
	...
	int reg01;
	if (jack_insert) {
	       reg01==snd_soc_read(codec,RT5639_SPK_VOL);
	...
	struct snd_pcm_hw_params *params, struct snd_soc_dai *dai)
{
	...
	if (dai_sel & RT5639_U_IF2) {
		...
		snd_soc_update_bits(codec, RT5639_I2S2_SDP,
			RT5639_I2S_DL_MASK, val_len);
		snd_soc_update_bits(codec, RT5639_ADDA_CLK1, mask_clk, val_clk);
		snd_soc_update_bits(codec, RT5639_ADDA_CLK1, 0xffff, 0x4414);
	}
	rt5639_reg_phone_init(codec);
	return 0;
}
...
static int rt5639_set_dai_fmt(struct snd_soc_dai *dai, unsigned int fmt)
{
	...
	if (dai_sel & RT5639_U_IF2) {
		snd_soc_update_bits(codec, RT5639_I2S2_SDP,
			RT5639_I2S_MS_MASK | RT5639_I2S_BP_MASK |
			RT5639_I2S_DF_MASK | RT5639_I2S2_SDI_MASK, reg_val  | RT5639_I2S2_SDI_I2S1);
	}
...
static int rt5639_voice_set_dai_sysclk(struct snd_soc_dai *dai,
		int clk_id, unsigned int freq, int dir)
{
	struct snd_soc_codec *codec = dai->codec;
	struct rt5639_priv *rt5639 = snd_soc_codec_get_drvdata(codec);
	...
	rt5639->sysclk = freq;
	rt5639->sysclk_src = clk_id;
	ret = snd_soc_read(codec, RT5639_GLB_CLK);
	dev_dbg(dai->dev, "Sysclk is %dHz and clock id is %d\n", freq, clk_id);
...
static int rt5639_voice_set_dai_pll(struct snd_soc_dai *dai, int pll_id, int source,
			unsigned int freq_in, unsigned int freq_out)
{
	struct snd_soc_codec *codec = dai->codec;
	struct rt5639_priv *rt5639 = snd_soc_codec_get_drvdata(codec);
	...
	dev_dbg(codec->dev, "bypass=%d m=%d n=%d k=%d\n", pll_code.m_bp,
		(pll_code.m_bp ? 0 : pll_code.m_code),
		pll_code.n_code, pll_code.k_code);
	printk("%s,bypass=%d m=%d n=%d k=%d\n",__func__, pll_code.m_bp,
		(pll_code.m_bp ? 0 : pll_code.m_code),
		pll_code.n_code, pll_code.k_code);
	snd_soc_write(codec, RT5639_PLL_CTRL1,
		pll_code.n_code << RT5639_PLL_N_SFT | pll_code.k_code);
...
struct snd_soc_dai_ops rt5639_aif2_dai_ops = {
	.hw_params = rt5639_voice_hw_params,
	.prepare = rt5639_prepare,
	.set_fmt = rt5639_set_dai_fmt,
	.set_sysclk = rt5639_voice_set_dai_sysclk,
	.set_pll = rt5639_voice_set_dai_pll,
};
