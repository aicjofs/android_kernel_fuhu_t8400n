...
struct lp3367_config {
	...
	struct lp3367_led_config led_config[2];
};

struct lp3367_power_rail {
	struct regulator *v_in;
	struct regulator *v_i2c;
};

struct lp3367_platform_data {
	struct lp3367_config config;
	u32 type; /* flash device type, refer to lp3367_type */
	...
	int (*power_on_callback)(struct lp3367_power_rail *pw);
	int (*power_off_callback)(struct lp3367_power_rail *pw);
};
